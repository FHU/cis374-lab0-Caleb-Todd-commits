...
string path = args[0];
int iterations = args.Length >= 2 && int.TryParse(args[1], out var it) ? it
: 50_000;
...
var universe = sorted.Range("a", "\uFFFF", int.MaxValue).ToList(); 
// all words (already sorted)
if (universe.Count == 0)
{
...
// Warm up JIT + caches
WarmUp(hash, sorted, containsQueries, prefixQueries, rangeQueries,
neighborQueries, k);
Console.WriteLine();
...
// --- workloads ---
private static string[] BuildContainsWorkload(List<string> universe, int
iterations)
{
...
private static string[] BuildPrefixWorkload(List<string> universe, int
iterations)
{
...
private static (string lo, string hi)[] BuildRangeWorkload(List<string>
universe, int iterations)
{
...
private static string[] BuildNeighborWorkload(List<string> universe, int
iterations)
{
...
queries[i] = (i % 5 == 0) ? w + "a" : w; 
// some not present, still meaningful for Next/Prev
}
return queries;
}
...
private static long RunRange(IWordSet dict, (string lo, string hi)[] ranges,
int k)
{
long sum = 0;
